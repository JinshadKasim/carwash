...
def emp_employee_login(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # print(employeeObj)
            employeeObj = Employees.objects.get(emp_phone=username,emp_password=password)
    ...
    if request.method == 'POST':
        ...
        try:
            if empObj.emp_password == oldpassword:
                Employees.objects.filter(id=request.session['empId']).update(emp_password=newpassword)
                return render(request,'employee/change_password.html',({'message':'Password Changed Successfully'}))
